...
class HelloradioLoader extends LagomApplicationLoader {
  ...
  override def load(context: LagomApplicationContext): LagomApplication =
    new HelloradioApplication(context) {
      override def serviceLocator: ServiceLocator = NoServiceLocator
    }

  override def loadDevMode(context: LagomApplicationContext): LagomApplication =
    new HelloradioApplication(context) with LagomDevModeComponents

  override def describeService = Some(readDescriptor[HelloradioService])
}
